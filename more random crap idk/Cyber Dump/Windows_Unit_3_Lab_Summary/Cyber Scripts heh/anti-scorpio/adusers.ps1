...
if ($confirm -eq "N") {
    Write-Host "Configure users manually... :("
    exit
}

$pass = ConvertTo-SecureString (Read-Host -prompt "Enter the password you will use") -AsPlainText -Force
$admingroups = get-adgroup -filter * | select-string -pattern "Admin"

$myusers = Get-ADUser -Filter "Name -ne 'krbtgt'" | %{$_.Name} | Select-String -NotMatch $env:USERNAME
foreach ($user in $myusers) {
    Unlock-ADAccount -Identity "$user"
    Set-ADUser -Identity "$user" -AllowReversiblePasswordEncryption $false -CannotChangePassword $false -ChangePasswordAtLogon $true -Enabled $true -PasswordNeverExpires $false -PasswordNotRequired $false
    Set-ADAccountPassword "$user" -NewPassword $pass -Reset
}
Set-ADUser -Identity "$env:username" -AllowReversiblePasswordEncryption $false -CannotChangePassword $false -Enabled $true -PasswordNeverExpires $false

$standardugroups = @()
foreach ($user in $standards) {$standardugroups += (get-aduser -identity $user -properties memberof)}
foreach ($user in $standardugroups) {
    foreach ($group in $admingroups) {
        if ($user.memberof -eq $group) {
            $cleangroup = "$group".Split(',')[0] -replace '^cn='
            $cleanname = $user.Name
            Remove-ADGroupMember -Identity $cleangroup -Members $user -Confirm:$false
            Write-Host "Removed $cleanname from $cleangroup" -ForegroundColor Red
        }
    }
}

$myusers += "$env:username"
$difference = diff $users $myusers -CaseSensitive
foreach ($r in $difference) {
    if ($r.SideIndicator -eq "<=") {
        $enable = $r.InputObject
        Write-Host "User $enable has been recognized as a to-be-added user. Double-check and configure this user manually." -ForegroundColor Cyan
    }
    elseif ($r.SideIndicator -eq "=>") {
        $disable = $r.InputObject
        Disable-ADAccount "$disable"
        Write-Host "$disable has been disabled" -ForegroundColor Red
    }
}
Stop-Transcript
Write-Host "User configuration complete. Check other groups." -ForegroundColor Green
pause